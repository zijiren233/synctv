...
impl Emby for EmbyService {
    ...
    async fn delete_active_encodeings(
        &self,
        request: Request<DeleteActiveEncodeingsReq>,
    ) -> Result<Response<Empty>, Status> {
        let req = request.into_inner();
        let resp = self.service.delete_active_encodeings(req).await
            .map_err(|e| Status::internal(format!("delete_active_encodeings failed: {e}")))?;
        Ok(Response::new(resp))
    }
}
