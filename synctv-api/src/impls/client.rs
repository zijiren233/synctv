...
/// Minimum password length when setting a room password.
const PASSWORD_MIN_LEN: usize = 4;
/// Maximum password length for any password input (setting or verifying).
...
fn validate_password_for_set(password: &str) -> Result<(), String> {
    if password.len() < PASSWORD_MIN_LEN {
        return Err(format!("Password too short (minimum {PASSWORD_MIN_LEN} characters)"));
    }
    if password.len() > PASSWORD_MAX_LEN {
        return Err(format!("Password too long (maximum {PASSWORD_MAX_LEN} characters)"));
    }
    Ok(())
}
...
mod tests {
    ...
    fn test_validate_password_for_set_boundary() {
        ...
        assert!(validate_password_for_set(&"a".repeat(PASSWORD_MIN_LEN)).is_ok());
        // One below minimum
        assert!(validate_password_for_set(&"a".repeat(PASSWORD_MIN_LEN - 1)).is_err());
        // Exactly maximum length
        assert!(validate_password_for_set(&"a".repeat(PASSWORD_MAX_LEN)).is_ok());
        // One above maximum
        assert!(validate_password_for_set(&"a".repeat(PASSWORD_MAX_LEN + 1)).is_err());
    }
